...
def executeSnowflakeQuery(prompt, user, password, account, warehouse, database, schema):
    ...
    st.write("debug in executeSnowflakeQuery starts here")
    st.write(prompt)

    snowflakeSQL = getSnowflakeSQL(prompt)

    st.write(snowflakeSQL) #the line above didn't run
...
def getTableSample(sampleSize, table):
    st.write("debug in getTableSample starts here")
    st.write(table)
    st.write(sampleSize)
...
def process_tables(dictionary, selectedTables, sampleSize):
    ...
    for table in selectedTables:
        ...
        st.write("debug in process_tables starts here")
        st.write(tableDescription)
...
def get_data_definitions_and_suggestions():
    with st.spinner("Getting table definitions..."):
        ...
        st.write("debug in get_data_definitions_and_suggestions starts here1")
